...
const BuildWithStackVisual = dynamic<VisualProps>(() => import("./BuildWithStackVisual"), { ssr: false });
const GenerateWithAIVisual = dynamic<VisualProps>(() => import("./GenerateWithAIVisual"), { ssr: false });
const CustomizeVisual      = dynamic<VisualProps>(() => import("./CustomizeVisual"),      { ssr: false });
const LaunchVisual         = dynamic<VisualProps>(() => import("./LaunchVisual"),         { ssr: false });
    ...
    <section id="features" className="pt-12 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div ref={containerRef} className="w-full flex flex-col items-center gap-y-30 pb-30">
          {features.map(({ color, badge, title, titleColor, desc, customVisual }, index) => {
                  ...
                  <div className="flex h-full w-full flex-col md:flex-row md:justify-between">
                    ...
                    <div className="relative -mr-1 h-[40%] max-h-125 w-full min-w-0 px-0 md:h-full md:min-w-0 md:flex-1 md:basis-0 md:p-0">
                      <div className="relative w-full h-70 md:h-full max-h-125" aria-hidden="true">
                        ...
                        {customVisual === "launch"    && <LaunchVisual         isActive={isActive} />}
                      </div>
                    </div>
                  </div>
                </div>
