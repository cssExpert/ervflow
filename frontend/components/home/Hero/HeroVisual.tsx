      ...
      <div className="w-full rounded-xl overflow-hidden bg-white dark:bg-zinc-900 border border-black/15 dark:border-zinc-800 shadow-2xl shadow-zinc-400/25 dark:shadow-black/70 ring-1 ring-inset ring-white/4">
            ...
            {(["desktop", "tablet", "mobile"] as Preview[]).map((s) => {
              ...
              return (
                <button
                  ...
                  aria-label={s.charAt(0).toUpperCase() + s.slice(1) + " preview"}
        ...
        <div className="relative flex h-125 md:h-150">
          ...
          <div className="@container relative flex-1 bg-zinc-100 dark:bg-zinc-950 overflow-hidden">
            ...
            <div
              className={`absolute inset-0 bg-zinc-50 dark:bg-zinc-950 transition-opacity duration-300 ${showCode ? "opacity-100 z-10" : "opacity-0 pointer-events-none z-0"}`}
            >
              <CodeView />
            </div>
...
function CodeView() {
  // Each token: light-mode class first, dark: override second
  const kw  = "text-purple-700 dark:text-purple-400";   // keywords
  const fn  = "text-amber-600  dark:text-yellow-300";   // function names
  const str = "text-emerald-700 dark:text-green-400";   // strings
  const tag = "text-blue-700   dark:text-blue-400";     // JSX tags
  const atr = "text-teal-700   dark:text-cyan-300";     // attributes
  const dflt= "text-zinc-700   dark:text-zinc-200";     // default code
  const cmt = "text-zinc-400   dark:text-zinc-500";     // comments

  const tokens = [
    { c: cmt,  v: "// components/Pricing.tsx — generated by ERVFlow AI\n" },
    { c: kw,   v: "import " },
    { c: dflt, v: "{ useState } " },
    { c: kw,   v: "from " },
    { c: str,  v: "'react'\n\n" },
    { c: kw,   v: "export function " },
    { c: fn,   v: "Pricing" },
    { c: dflt, v: "() {\n" },
    { c: dflt, v: "  " },
    { c: kw,   v: "const " },
    { c: dflt, v: "[annual, setAnnual] = " },
    { c: fn,   v: "useState" },
    { c: dflt, v: "(false)\n\n" },
    { c: dflt, v: "  " },
    { c: kw,   v: "return " },
    ...
    { c: atr,  v: " className" },
    { c: dflt, v: "=" },
    { c: str,  v: '"grid grid-cols-3 gap-6"' },
    { c: tag,  v: ">\n" },
    { c: dflt, v: "        " },
    { c: tag,  v: "<PricingCard " },
    { c: atr,  v: "tier" },
  ...
  return (
    <div className="h-full p-4 font-mono text-xs leading-relaxed overflow-auto bg-zinc-50 dark:bg-zinc-950">
      {tokens.map((t, i) => (
